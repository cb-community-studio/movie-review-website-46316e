...
import ratingsFakerFactory from "./ratingsFakerFactory";
...
const RatingsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = ratingsFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("ratings").create(elem));
...
export default connect(null, mapDispatch)(RatingsFakerDialogComponent);
